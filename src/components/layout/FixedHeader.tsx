...
export default function FixedHeader({ title, subtitle, children, className = '' }: FixedHeaderProps) {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${className}`}>
      <div className={`bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700 transition-all duration-300 ${
        ...
        <div className="px-4 max-w-6xl mx-auto">
          <div className="flex items-center justify-between">
            <div className={`transition-all duration-300 ${isScrolled ? 'scale-90' : 'scale-100'}`}>
              ...
              {subtitle && !isScrolled && (
                <p className="text-gray-600 dark:text-gray-400 text-sm mt-1 transition-opacity duration-300">
                  {subtitle}
                </p>
              )}
            </div>
