...
export default function SplitPayRedesigned() {
  ...
  const transformExpensesToEvents = useCallback((expenses: ExpenseWithDetails[], userId: string): ExpenseEvent[] => {
    ...
    eventGroups.forEach((groupExpenses, eventId) => {
      ...
      const myExpenses = groupExpenses.reduce((sum: number, exp: any) => {
  ...
  const transformExpenseEventsToEvents = useCallback((expenseEvents: ExpenseEventType[], userId: string): ExpenseEvent[] => {
    return expenseEvents.map(expenseEvent => {
      const totalAmount = expenseEvent.expenses?.reduce((sum: number, exp: any) => sum + Number(exp.amount), 0) || 0
      const myExpenses = expenseEvent.expenses?.reduce((sum: number, exp: any) => {
  ...
  useEffect(() => {
    ...
    fetchExpenses()
  }, [user, authLoading, transformExpensesToEvents, transformExpenseEventsToEvents])
